...
class SeasonFixtureShedule(CalendarBase):
    ...
    def get_fixtures(self, with_week: bool=False):
        ...
        for week, entries in self.weeks.items():
            if with_week:
                all_fixtures.extend([(week, entry) for entry in entries if not isinstance(entry, Result)])
            else:
                all_fixtures.extend([entry for entry in entries if not isinstance(entry, Result)])
        return all_fixtures
    ...
    def get_results(self, with_week: bool=False):
        ...
        for week, entries in self.weeks.items():
            if with_week:
                all_results.extend([(week, entry) for entry in entries if isinstance(entry, Result)])
            else:
                all_results.extend([entry for entry in entries if isinstance(entry, Result)])
        return all_results
